...
+ (NSURLSessionTask *)POST:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:[RSAAndDESEncrypt encryptParams:parameters token:YES] progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        success ? success(responseObject) : nil;
        //对数据进行异步缓存
//        if ([parameters[@"page"] isEqualToString:@"1"]) {
            responseCache ? [PPNetworkCache setHttpCache:responseObject URL:URL parameters:parameters] : nil;
//        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        PPLog(@"%@",_isOpenLog ? NSStringFormat(@"error = %@",error) : @"PPNetworkHelper已关闭日志打印");
