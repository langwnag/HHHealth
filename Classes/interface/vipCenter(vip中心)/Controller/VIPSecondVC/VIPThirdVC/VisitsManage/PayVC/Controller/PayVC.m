    ...
    [VDNetRequest HH_RequestHandle:paramsDic
                    ...
                    viewController:self
                           success:^(id responseObject) {
                               ...
                               SZRLog(@" 👌😁[RSAAndDESEncrypt DESDecrypt:responseObject[DATA]] %@",[RSAAndDESEncrypt DESDecrypt:responseObject[DATA]]);
                               VD_ShowBGBackError(NO);
                               
                               if ([model.paymentId isEqual:@1]) {
...
- (void)wechat:(NSDictionary* )dict {
    ...
    [DWQPAYMANAGER dwq_payWithOrderMessage:req
                                  callBack:^(DWQErrCode errCode, NSString *errStr) {
                                      if (errCode == DWQErrCodeSuccess) {
                                          [self sendNotificationToChatViewController];
...
- (void)aliPayWithBody:(NSString *)body{
    ...
    __weak PayVC * weakSelf = self;
    [DWQPAYMANAGER dwq_payWithOrderMessage:orderMessage
                                  callBack:^(DWQErrCode errCode, NSString *errStr) {
                                      ...
                                      if (errCode == DWQErrCodeSuccess) {
                                          VisitsManageVC* mangerVC = [VisitsManageVC new];
                                          [weakSelf.navigationController pushViewController:mangerVC animated:YES];
                                          [weakSelf sendNotificationToChatViewController];
                                          if (weakSelf.passPayStatesBlock) {
                                              weakSelf.passPayStatesBlock(payState);
                                          }
...
- (void)sendNotificationToChatViewController{
    
    [[NSUserDefaults standardUserDefaults] setObject:self.homeServiceId forKey:@"LY_SERVICE_ID"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"OrderPaySuccessful" object:self.homeServiceId];
}
