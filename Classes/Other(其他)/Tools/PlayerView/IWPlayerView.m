...
static const CGFloat IWPlayerAnimationTimeInterval = 7.0f;
...
/** 蒙版View */
@property (nonatomic, strong) IWPlayerMaskView *maskView;
/**  用来保存快进的总时长 */
...
- (void)layoutSubviews
{
    ...
    [self.maskView addSubview:self.backBtn];
    [self.backBtn setImage:[UIImage imageNamed:@"play_back_full-1"] forState:UIControlStateNormal];
    ...
    [self.iconView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.horizontalLabel);
        make.bottom.equalTo(self.horizontalLabel.mas_top);
...
- (void)setVideoURL:(NSURL *)videoURL
{
    ...
    self.maskView = [[IWPlayerMaskView alloc] initWithFrame:self.frame];
//    [self insertSubview:self.maskView belowSubview:self.backBtn];
    [self addSubview:self.maskView];
    self.maskView.player = self.player;
    ...
    [self.activity startAnimating];
     // 传入名字
    self.maskView.titleStr = self.nameStr;
}
...
- (void)autoFadeOutControlBar
{
    ...
    [self performSelector:@selector(hideMaskView) withObject:nil afterDelay:IWPlayerAnimationTimeInterval];
...
- (void)playerTimerAction
{
    if (_playerItem.duration.timescale != 0) {
        ...
        NSInteger proHou = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale % 60 / 60;//总小时
        
        //duration 总时长
        NSInteger durMin = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale / 60;//总秒
        NSInteger durSec = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale % 60;//总分钟
        NSInteger durHou = (NSInteger)_playerItem.duration.value / _playerItem.duration.timescale % 60 / 60;//总小时
        
        self.maskView.currentTimeLabel.text = [NSString stringWithFormat:@"%02ld:%02ld:%02ld",(long)proHou,(long)proMin, (long)proSec];
        self.maskView.totalTimeLabel.text = [NSString stringWithFormat:@"/%02ld:%02ld:%02ld", (long)durHou,(long)durMin, (long)durSec];
    }
}
...
- (void)startAction:(UIButton *)button
{
    if (button.selected) {
        [_player play];
        [button setImage:[UIImage imageNamed:@"bnt_stop"] forState:UIControlStateNormal];
        
    } else {
        [_player pause];
        [button setImage:[UIImage imageNamed:@"start_play"] forState:UIControlStateNormal];
...
- (void)horizontalMoved:(CGFloat)value
{
    ...
    //快进/后退图标
    self.iconView.image = [UIImage imageNamed:style];
    ...
    //富文本设置/后面颜色属性
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:self.horizontalLabel.text];
    NSString * time = [self.horizontalLabel.text componentsSeparatedByString:@"/"][1];
    NSRange range = [self.horizontalLabel.text rangeOfString:time];
    [string setAttributes:@{NSForegroundColorAttributeName:[UIColor orangeColor]} range:range];
    self.horizontalLabel.attributedText = string;
...
{
    // 获取小时
    NSString *hou = [NSString stringWithFormat:@"%02d",time / 60 / 60];
    // 获取分钟
