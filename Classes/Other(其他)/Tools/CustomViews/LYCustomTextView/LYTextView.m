...
- (UITextView *)textView{
    if (!_textView) {
        ...
        _textView.textColor = HEXCOLOR(0x989898);
...
- (void)setHasMasksToBounds:(BOOL)hasMasksToBounds{
    _hasMasksToBounds = hasMasksToBounds;
    if (hasMasksToBounds) {
        self.layer.cornerRadius = 8.0f;
        self.layer.masksToBounds = YES;
    }else{
        self.layer.cornerRadius = 0.00f;
    }
}
...
- (void)setLeadingSpace:(CGFloat)leadingSpace{
    ...
    CGRect rect = self.placeHolderLab.frame;
    rect.origin.x = leadingSpace;
    rect.size.width = self.frame.size.width - leadingSpace* 2;
    self.placeHolderLab.frame = rect;
    
    CGRect textRect = self.textView.frame;
    textRect.origin.x = leadingSpace;
    textRect.size.width = self.frame.size.width - leadingSpace* 2;
